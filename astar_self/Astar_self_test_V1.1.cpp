...
std::vector<std::string> pathPlan_Astar(const std::vector< std::vector<GridCell> >& grid_map, std::string start_point, std::string target_point)
{
  ...
  while(!openList.empty())
  {
    ...
    for(int i = 0; i < 4; i++)
    {
      ...
      np_x += delta[0][i];
      np_y += delta[1][i];
      std::cout << "np_x:" << np_x << " " << "np_y:" << np_y << std::endl;

      if(np_x < map_size_x && np_y < map_size_y &&
         np_x >= 0 && np_y >= 0 &&
         grid_map[np_x][np_y].value == 0)
      {
        std::cout << "inter_first" << std::endl;
        Node next(np_x, np_y, best.g + dg);
        if(info[np_x][np_y] == 0)
        {
          std::cout << "inter_second" << std::endl;
          next.ManhattanDistance(tp_x, tp_y);
          next.F();
        ...
        if(info[np_x][np_y] == 1)
        {
          ...
          for(list_iterator iter = openList.begin(); iter != openList.end(); iter++)
          {
            ...
            {
              std::cout << "\n" << std::endl;
              std::cout << "renew:" << (*iter).px << "_" <<(*iter).py << std::endl;
              std::cout << "src:" << (*iter).father[0][0] << "_" <<(*iter).father[0][1] << std::endl;
              (*iter).g = temp_g;
              (*iter).F();
              (*iter).father[0][0] = best.px;
              (*iter).father[0][1] = best.py;
              std::cout << "renew:" << (*iter).father[0][0] << "_" <<(*iter).father[0][1] << std::endl;
              std::cout << "\n" << std::endl;
            }
          }
        }
